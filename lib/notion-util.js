import { Client } from "@notionhq/client";

function createSlug(text) {
  return text
    .toLowerCase()
    .replace(/[^a-zA-Z0-9 ]/g, "")
    .trim()
    .replace(/\s+/g, "-");
}

export async function getPostData(page, content) {
  let body = "";

  //console.log(content.results);

  content.results.forEach((result) => {
    if (result.type == "heading_1") {
      body += "# ";
    } else if (result.type == "heading_2") {
      body += "## ";
    } else if (result.type == "heading_3") {
      body += "### ";
    } else if (result.type == "bulleted_list_item") {
      body += "- ";
    } else if (result.type == "numbered_list_item") {
      body += "- ";
    }
    if (result[result.type].rich_text?.length > 0) {
      body += result[result.type].rich_text[0]?.plain_text;
    }

    body += "\n";
    if (result.type == "paragraph") {
      body += "\n";
    }
  });

  // console.log(body);

  const postData = {
    slug: createSlug(page.properties.Title.title[0].plain_text),
    title: page.properties.Title.title[0].plain_text,
    date: page.created_time,
    image: page.properties["Main Image"].rich_text[0].plain_text.trim(),
    excerpt: page.properties.Summary.rich_text[0].plain_text,
    isFeatured: true,
    content: body,
  };

  return postData;
}

export async function getAllNotionPages() {
  const notion = new Client({ auth: process.env.NOTION_API_KEY });
  const pages = [];

  const databaseId = process.env.NOTION_DATABASE_ID;
  const response = await notion.databases.query({
    database_id: databaseId,
    filter: {
      or: [
        {
          property: "Done",
          checkbox: {
            equals: true,
          },
        },
      ],
    },
    sorts: [
      {
        property: "Publish Time",
        direction: "ascending",
      },
    ],
  });

  for (const result of response.results) {
    const content = await notion.blocks.children.list({
      block_id: result.id,
    });
    pages.push(await getPostData(result, content));
  }

  //console.log(pages);

  return pages;
}

export async function getFeaturedNotionPages() {
  const notion = new Client({ auth: process.env.NOTION_API_KEY });
  const pages = [];

  const databaseId = process.env.NOTION_DATABASE_ID;
  const response = await notion.databases.query({
    database_id: databaseId,
    filter: {
      and: [
        {
          property: "Done",
          checkbox: {
            equals: true,
          },
        },
        {
          property: "Featured",
          checkbox: {
            equals: true,
          },
        },
      ],
    },
    sorts: [
      {
        property: "Publish Time",
        direction: "ascending",
      },
    ],
  });

  for (const result of response.results) {
    const content = await notion.blocks.children.list({
      block_id: result.id,
    });
    pages.push(await getPostData(result, content));
  }

  return pages;
}

export async function getAboutNotionPages() {
  const notion = new Client({ auth: process.env.NOTION_API_KEY });
  const pages = [];

  const databaseId = process.env.NOTION_DATABASE_ID;
  const response = await notion.databases.query({
    database_id: databaseId,
    filter: {
      and: [
        {
          property: "Done",
          checkbox: {
            equals: true,
          },
        },
        {
          property: "Category",
          select: {
            equals: "about",
          },
        },
      ],
    },
    sorts: [
      {
        property: "Publish Time",
        direction: "ascending",
      },
    ],
  });

  for (const result of response.results) {
    const content = await notion.blocks.children.list({
      block_id: result.id,
    });
    pages.push(await getPostData(result, content));
  }

  return pages;
}

export async function getProfileNotionPages() {
  const notion = new Client({ auth: process.env.NOTION_API_KEY });
  const pages = [];

  const databaseId = process.env.NOTION_DATABASE_ID;
  const response = await notion.databases.query({
    database_id: databaseId,
    filter: {
      and: [
        {
          property: "Done",
          checkbox: {
            equals: true,
          },
        },
        {
          property: "Category",
          select: {
            equals: "profile",
          },
        },
      ],
    },
    sorts: [
      {
        property: "Publish Time",
        direction: "ascending",
      },
    ],
  });

  for (const result of response.results) {
    const content = await notion.blocks.children.list({
      block_id: result.id,
    });
    pages.push(await getPostData(result, content));
  }

  return pages;
}
